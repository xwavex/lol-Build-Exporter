...
function processData(itemSite) {
  if (itemSite != null) {
	...
	// find name and before ":{"name":" teil davor bis " suchen, dass ist dann die id
...
$(document).ready(function(){
  ...
  if (allDivs.length > 0) { // only do further stuff, when button can be added properly!
	...
	btn.onclick = function() {
	  ...
	  var activeIndex = 0;
	  if ((allBuildBoxes != null) && (allBuildBoxes.length > 0)) {
	...
	//get champ
	var champA = allBuildBoxes[activeIndex].getElementsByTagName('a');
	for (var j = 0; j < champA.length; j++) {
	  if (champA[j].className.indexOf("ajax-tooltip {t:\'Champion\',i:\'") != -1) {
		champion = champA[j].innerHTML.trim();
	  }
	}
	...
	listItemBlocks = new Array();
	  ...
	  for (var j = 0; j < allItems.length; j++) {
		...
		for (var l = 0; l < innerItemSpans.length; l++) {
		  ...
		  } else if (innerItemSpans[l].className == "hiliteW") {
	...
	for (var k = 0; k < allDivs.length; k++) {
	  if (allDivs[k].className == "guide-details") {
